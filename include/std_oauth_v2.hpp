   ...
   namespace oauth_v2
   {
      ...
      public:
         ...
         */
         ...
         string request_auth_code(string const & auth_server,
                                  string const & scope,
                                  string const & state);
         string access_auth_code(string const & auth_server,
                                 string const & auth_code,
                                 string const & scope);
         string access_resource_owner(string const & auth_server,
                                      string const & user_name,
                                      string const & password);
         string access_refresh_token(string const & refresh_token);
         string request(string const & uri, string const & params);
